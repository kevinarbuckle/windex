...
tree<-trees
if (is.phylo(trees)==T) trees<-c(trees,trees)
ages<-c()
...
if (is.phylo(tree)==T) warning("This distribution is based on a single age estimate as only one tree (not a set) was provided")
}
if (return.ages==T) {
if (is.phylo(tree)==T) ages[1]
else ages
}
}
